    ...
    try {
        ...
        groups.forEach(group => {
            ...
            if (jours.length === 7) {
                ...
                // Plage continue : "Lun — Ven"
                jourText = `${jourShort[jours[0]]} — ${jourShort[jours[jours.length - 1]]}`;
            } else if (jours.length === 2) {
                jourText = `${jourLabels[jours[0]]} & ${jourLabels[jours[1]]}`;
            } else {
                jourText = jourLabels[jours[0]];
            }

            if (!group.data.ouvert) {
                html += `<p class="mb-1"><strong>${jourText}</strong> : Fermé</p>`;
            } else {
                const slots = [];
                ...
                html += `<p class="mb-1"><strong>${jourText}</strong><br>${slots.join(' | ')}</p>`;
            }
        });
