// frontend/js/shared-dashboard.js
// Fonctions partagées entre employe-dashboard et admin-dashboard
...
function showEmployeSuccess(message) { showDashboardSuccess(message); }
function showEmployeError(message) { showDashboardError(message); }
...
async function confirmStatusChange() {
    ...
    try {
        ...
        if (result.success) {
            ...
            showDashboardSuccess(`Statut mis à jour : ${getStatusText(newStatus)}`);
            refreshCommands();
        } else {
            showDashboardError(result.message || 'Erreur lors de la mise à jour du statut');
        }
    } catch (error) {
        console.error('Erreur:', error);
        showDashboardError('Erreur de communication avec le serveur');
    } finally {
        btn.disabled = false;
...
async function confirmCancelOrder() {
    ...
    if (!contactMode) {
        showDashboardError('Veuillez sélectionner le mode de contact utilisé');
        return;
    }
    if (!motif || motif.length < 10) {
        showDashboardError('Le motif doit contenir au moins 10 caractères');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            showDashboardSuccess('Commande annulée avec succès');
            refreshCommands();
        } else {
            showDashboardError(result.message || 'Erreur lors de l\'annulation');
        }
    } catch (error) {
        console.error('Erreur:', error);
        showDashboardError('Erreur de communication avec le serveur');
...
async function loadAllMenus() {
    ...
    try {
        ...
        if (!result.success) {
            showDashboardError('Erreur lors du chargement des menus');
            return;
        }
        ...
        if (loadingState) loadingState.style.display = 'none';
        showDashboardError('Erreur de connexion au serveur');
    }
}
...
async function submitMenuForm() {
    ...
    try {
        ...
        if (result.success) {
            showDashboardSuccess(isEdit ? 'Menu modifié avec succès' : 'Menu créé avec succès');
            const modal = bootstrap.Modal.getInstance(document.getElementById('menuFormModal'));
            if (modal) modal.hide();
            loadAllMenus();
        } else {
            showDashboardError(result.message || 'Erreur lors de l\'opération');
        }
...
async function confirmDeleteMenu() {
    ...
    try {
        ...
        if (result.success) {
            showDashboardSuccess(result.message || 'Menu désactivé avec succès');
            const modal = bootstrap.Modal.getInstance(document.getElementById('menuDeleteModal'));
            if (modal) modal.hide();
            loadAllMenus();
        } else {
            showDashboardError(result.message || 'Erreur lors de la désactivation');
        }
...
async function loadAllPlats() {
    ...
    try {
        ...
        if (!result.success) {
            showDashboardError('Erreur lors du chargement des plats');
            return;
        }
...
async function submitPlatForm() {
    ...
    if (!nom || !type) {
        showDashboardError('Veuillez remplir le nom et le type du plat');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showDashboardSuccess(isEdit ? 'Plat modifié avec succès' : 'Plat créé avec succès');
...
async function confirmDeletePlat() {
    ...
    try {
        ...
        if (result.success) {
            showDashboardSuccess(result.message || 'Plat supprimé avec succès');
            ...
            showDashboardError(result.message || 'Erreur lors de la suppression');
        }
...
async function loadHoraires() {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            showDashboardError('Erreur lors du chargement des horaires');
        }
    } catch (error) {
...
async function submitHoraireEdit() {
    ...
    try {
        ...
        if (result.success) {
            showDashboardSuccess(result.message || 'Horaires mis à jour');
            const modal = bootstrap.Modal.getInstance(document.getElementById('horaireEditModal'));
            if (modal) modal.hide();
            loadHoraires();
        } else {
            showDashboardError(result.message || 'Erreur lors de la mise à jour');
        }
    } catch (error) {
...
async function validateAvis(avisId, valide) {
    ...
    try {
        ...
        if (result.success) {
            showDashboardSuccess(result.message);
            loadAllAvis();
        } else {
            showDashboardError(result.message || 'Erreur lors de la validation');
        }
    } catch (error) {
...
// ============================================
// Navigation générique entre sections
// ============================================
function showSection(section) {
    // Récupérer la config du dashboard courant
    ...
    const titleEl = document.getElementById(config.titleElementId);
    if (titleEl) titleEl.textContent = config.titles[section] || '';
    ...
    if (siteHeader) {
        if (section === 'commandes' && config.filtersHeaderHTML) {
            siteHeader.innerHTML = config.filtersHeaderHTML;
        } else {
    ...
    const headerActions = document.getElementById('employe-header-actions') || document.getElementById('admin-header-actions');
    if (headerActions) {
...
function closeDashboardMenu() {
    const offcanvasEl = document.getElementById('employeDashboardMobileMenu') || document.getElementById('adminDashboardMobileMenu');
    if (offcanvasEl) {
        const offcanvas = bootstrap.Offcanvas.getInstance(offcanvasEl);
        if (offcanvas) offcanvas.hide();
    }
...
function closeEmployeMenu() { closeDashboardMenu(); }
...
// Alias pour compatibilité
function employeLogout() { dashboardLogout(); }
